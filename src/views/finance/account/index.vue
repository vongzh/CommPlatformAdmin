<template>
  <div class="content">
    <el-tabs v-model="tab" :stretch="true" @tab-click="tabClick">
      <el-tab-pane label="交易明细" name="transcation">
              ...
              <el-card>
                <el-form ref="searchForm" :inline="true" :model="searchCondition" label-position="top" class="searchForm">
                    ...
                    <el-select v-model="searchCondition.sourcePlatformId" placeholder="第三方平台" clearable>
                      <el-option  v-for="item in merchantsDict " :label="item.dictLabel"  :value="item.dictValue" :key="item.dictValue" />
                    </el-select>
                  </el-form-item>
                  <el-form-item label="酒店/房型名称" prop="productName">
                    <el-input v-model="searchCondition.productName" clearable placeholder="请输入酒店/房型名称" class="filter-item input-search" />
                  </el-form-item>
                  <el-form-item label="订单号" prop="orderNo">
                    <el-input v-model="searchCondition.orderNo" clearable placeholder="请输入订单号" class="filter-item input-search" />
                  </el-form-item>
                  ...
                  <el-form-item label="支付单号" prop="paymentOrderNo">
                    <el-input v-model="searchCondition.paymentOrderNo" clearable placeholder="请输入支付单号" class="filter-item input-search" />
                  </el-form-item>
                  <el-form-item label="交易时间" prop="transactionTimes" style="width: 230px;">
                    <el-date-picker
                      v-model="searchCondition.transactionTimes"
                  ...
                  <el-button v-waves class="filter-item" type="primary" size="small" icon="el-icon-search" @click="searchTranscation">搜索</el-button>
        ...
        <el-card>
          <div slot="header">
            ...
            <span style="font-size: 14px;margin-left:2rem">收入：<i style="font-size: large;color: #1890FF;">￥{{ totalTranscation.item1 }}</i></span>
            <span style="font-size: 14px;margin-left: 1rem;">支出：<i style="font-size: large;color: #1890FF;">￥{{ totalTranscation.item2 }}</i>
              <el-link type="text"  :underline="false" class="buttomstyle" @click="exportAccountTranscations">导出列表</el-link>
            </span>
          </div>
          <el-table v-loading="transactionLoading" :data="transactionList" :fit="true" border stripe highlight-current-row>
             <el-table-column prop="sourcePlatformId" label="第三方平台" min-width="7%">
            <template slot-scope="scope">
            <b>【{{ getPlatName(scope.row.sourcePlatformId) }}】</b>
            </template>
             </el-table-column>
      ...
      <el-tab-pane label="账户金额明细" name="recharge">
            <el-card>
              <el-form inline label-position="top" ref="searchForm1" :model="TSearchCondition" class="searchForm">
                      <el-form-item label="第三方平台" prop="sourcePlatformId">
                    <el-select v-model="TSearchCondition.sourcePlatformId" placeholder="第三方平台" clearable>
                      <el-option  v-for="item in merchantsDict " :label="item.dictLabel"  :value="item.dictValue" :key="item.dictValue" />
                    </el-select>
                  </el-form-item>
                <el-form-item label="交易单号" prop="accountTransactionNo">
                  <el-input v-model="TSearchCondition.accountTransactionNo" clearable placeholder="请输入交易单号" class="filter-item input-search" />
                </el-form-item>
                <el-form-item label="用户编号" prop="userId">
                  <el-input v-model="TSearchCondition.userId" clearable placeholder="请输入用户编号" class="filter-item input-search" />
                </el-form-item>
                <!-- <el-form-item label="用户名称" prop="userName">
              <el-input v-model="TSearchCondition.userName" clearable placeholder="请输入用户名称" class="filter-item input-search" />
        </el-form-item> -->
                ...
                <el-form-item label="充值账号" prop="transactionAccountNo">
                  <el-input v-model="TSearchCondition.transactionAccountNo" clearable placeholder="请输入充值账号" class="filter-item input-search" />
                </el-form-item>
                <el-form-item label="操作人编号" prop="creater">
                  <el-input v-model="TSearchCondition.creater" clearable placeholder="请输入操作人编号" class="filter-item input-search" />
                </el-form-item>
                <el-form-item label="操作时间" prop="transactionTimes" style="width: 230px;">
                  <el-date-picker
                    v-model="TSearchCondition.transactionTimes"
                    type="daterange"
                    value-format="yyyy-MM-dd"
                    :picker-options="pickerOptions"
                    range-separator="至"
                  ...
                  />
							...
							<el-form-item label="　" style="user-select: none;width: 200px;">
                <el-button v-waves class="filter-item" type="primary" size="small" icon="el-icon-search" @click="TsearchTranscation">搜索</el-button>
                <el-button v-waves class="filter-item" type="warning" size="small" icon="el-icon-close"  @click="handleReset('searchForm1')">重置</el-button>
							</el-form-item>
              </el-form>
            </el-card>
        <el-card class="data">
          <div slot="header">
            ...
            <el-link type="text" :underline="false" class="buttomstyle" @click="exportAccountTranscations2">导出列表</el-link>
          </div>
          <el-table v-loading="transactionLoading" :data="AccountTranscations" :fit="true" border stripe highlight-current-row>
             <el-table-column prop="sourcePlatformId" label="第三方平台" >
            <template slot-scope="scope">
            <b>【{{ getPlatName(scope.row.sourcePlatformId) }}】</b>
            </template>
             </el-table-column>
            ...
            <!-- <el-table-column prop="userPhone" label="注册手机号" /> -->
            ...
            <el-table-column prop="transactionTypeDesc" label="充值类型" />
            <el-table-column prop="transactionAccountNo" label="账号/银行账号" />
            <el-table-column prop="tradeNo" label="交易流水号" />
            <!-- <el-table-column prop="transactionAccountNo" label="充值账户" /> -->
            <!-- <el-table-column prop="bankName" label="银行名称" /> -->
            <el-table-column prop="operName" label="操作人" />
            <el-table-column prop="creater" label="操作人编号" />
          ...
          </el-table :data="accounttranscations"></el-card>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
import { GetTranscations, GetTotalTranscation,GetAccountTranscations } from '@/api/account/account'
...
export default {
  ...
  methods: {
    ...
    fetchTranscations(tip) {
      ...
      const data = { ...this.searchCondition1, transactionTime: {
      ...
      GetTranscations(data).then(res => {
        ...
        if (tip) {
          this.$message({ type: 'info', message: '搜索成功' })
        }
      })
    },
        //充值明细搜索
    ...
    if (type === 'refresh') {
      ...
      const data = { ...this.TSearchCondition1, transactionTime: {
      ...
      }, pageIndex: this.pageIndex2, pageSize: this.pageSize2 }
    ...
    fetchTotalTranscation() {
      GetTotalTranscation(this.searchCondition1).then(res => {
    ...
    exportAccountTranscations() {
      ...
      }, pageIndex: this.pageIndex, pageSize: 1000 }
      GetTranscations(data).then(res => {
        const orders = res.data.rows
        import('@/vendor/Export2Excel').then(excel => {
          const filterVal = ['sourcePlatformDesc','transcationNo', 'transcactionAmount', 'transcactionTypeDesc', 'payTypeDesc', 'productName', 'orderNo', 'paymentOrderNo', 'refundOrderNo', 'transcationTime']
          const data = this.formatJson(filterVal, orders)
          excel.export_json_to_excel({
            header: ['第三方平台','交易流水号', '交易金额', '交易类型', '支付方式', '产品名称', '订单号', '支付单号', '退款单号', '交易时间'],
            data: data,
    ...
    exportAccountTranscations2() {
      const data = { ...this.TSearchCondition1, transactionTime: {
        item1: this.TSearchCondition1.transactionTimes ? this.TSearchCondition1.transactionTimes[0] : '',
        item2: this.TSearchCondition1.transactionTimes ? this.TSearchCondition1.transactionTimes[1] : ''
      }, pageIndex: this.pageIndex2, pageSize: 1000 }
        GetAccountTranscations(data).then(res => {
          const orders = res.data.rows
          import('@/vendor/Export2Excel').then(excel => {
            const filterVal = ['sourcePlatformDesc','accountTransactionNo', 'userName', 'userId', 'transactionBeforeAmount',
              'transactionAmount', 'transactionAfterAmount', 'transactionTypeDesc', 'transactionAccountNo','tradeNo','operName','creater','createTime','remark'
            ]
            const data = this.formatJson(filterVal, orders)
            excel.export_json_to_excel({
              header: ['第三方平台','交易单号', '用户名', '用户编号', '原始金额', '交易金额', '剩余金额', '充值类型', '账号/银行账号','交易流水号','操作人','操作人编号','操作时间','备注信息'],
              data: data,
              filename: this.filename,
              autoWidth: this.autoWidth,
              bookType: this.bookType
