<template>
	...
	<el-form-item label="拒单状态" prop="rejectionStatus">
						<el-select v-model="searchCondition.rejectionStatus" clearable>
							<el-option v-for="os in RejectionStatus" :key="os.value" :label="os.text" :value="os.value" />
						</el-select>
					</el-form-item>
					...
					<el-form-item style="user-select: none;width: 100%;margin-top: 10px;text-align: center;">
        ...
        <el-button type="text" @click="exportOrders"><br></el-button>
			...
			<el-table ref="tableDataRef" v-loading="tableLoading" :data="orders" :fit="true" stripe
				...
				<el-table-column v-if="colData[0].istrue" label="订单号/酒店名称" min-width="8%">
          ...
          <template slot-scope="scope">
            ...
            <el-link slot="reference" type="info" @click="goHotel(scope.row.hotelCode)">{{ scope.row.hotelName}}</el-link>
				...
				<el-table-column prop="roomTypeName" label="房型" min-width="5%" v-if="colData[3].istrue" />
				<el-table-column prop="bookingDaysShow" label="间/夜" min-width="4%" v-if="colData[4].istrue" />

				<el-table-column label="入离日期" min-width="4%" v-if="colData[5].istrue">
					<template slot-scope="scope">
						{{ dateTimeToDate(scope.row.checkInDate).substr(5) }}<br />{{ dateTimeToDate(scope.row.checkOutDate).substr(5) }}
					</template>
				</el-table-column>
				<el-table-column prop="orderAmount" label="订单金额" min-width="4%" v-if="colData[6].istrue">
					<template slot-scope="scope">
						¥{{scope.row.orderAmount}}
					</template>
				</el-table-column>
				<el-table-column prop="checkInName" label="入住人" min-width="4%" v-if="colData[7].istrue" />
				<el-table-column prop="checkInPhone" label="入住人电话" min-width="6%" v-if="colData[8].istrue" />


				<!-- <el-table-column prop="invoiceDrawerDesc" label="开票方" /> -->

				<el-table-column prop="orderDate" label="预定日期" min-width="7%" v-if="colData[9].istrue" />
				<!-- <el-table-column prop="customerName" label="客户名称" />
        <el-table-column prop="customerNo" label="客户编号" /> -->
				<el-table-column label="订单状态" min-width="4%">
					<template slot-scope="scope">
						 
						...
						<font v-else-if="scope.row.orderStatus==OrderStatusEnum.RESERVEING||scope.row.orderStatus==OrderStatusEnum.WAITCONFIRM" style="color:red">
							{{ scope.row.orderStatusDesc }}</font>   
          ...
          <template slot-scope="scope">
            <el-tag effect="plain">{{ scope.row.refundStatusDesc }}</el-tag>
				...
				<el-table-column label="操作" min-width="4%" v-if="colData[11].istrue">
					<template slot-scope="scope">
						<el-link style="color:#DF9524" size="mini" @click="goOrder(scope.row.orderNo,1)">查看</el-link>
					</template>
				</el-table-column>
				<el-table-column label="处理时间" min-width="4%" v-if="colData[12].istrue">
					<template slot-scope="scope">
						{{ scope.row.remaining }}
					</template>
				</el-table-column>
				<el-table-column label="第三方订单号" min-width="8%" v-if="colData[13].istrue">
					<template slot-scope="scope">
						{{ scope.row.thirdOrderNo }}<span
							v-if="scope.row.thirdOrderNo">【{{ getPlatName(scope.row.sourcePlatformId)  }}】</span>
					</template>
				</el-table-column>
					<el-table-column prop="platform" label="数据渠道/开票方" min-width="7%" v-if="colData[15].istrue" >
					<template slot-scope="scope">
						{{ scope.row.platform }}
					    <span><b v-if="scope.row.invoiceTypeDesc">【{{scope.row.invoiceTypeDesc }}】</b> </span>
					</template>
					</el-table-column>
				<el-table-column prop="lockUserName" label="处理人" min-width="4%" v-if="colData[14].istrue" />
			</el-table>
		...
		methods: {
			...
			fetchOrders(tip) {
				...
				QueryBackOrders(condition).then(res => {
					...
					//     if (value.paymentStatus === this.OrderPaymentStatusEnum.PAYSUCCESS && value.orderStatus === this.OrderStatusEnum.WaitConfirm) {
					...
					//       value.remaining = minutes + '分' + seconds + '秒'
					//       if (months < 0 || days < 0 || hours < 0 || minutes < 0) {
					//         value.remaining = '-'
					//       }
					//       return value
			...
			//点击订单
			goOrder(orderNo, isdetails) {
				this.$router.push({
					path: `/manualorder/${orderNo}`,
					query: {
						isdetails: isdetails
					}
				})
			...
			exportOrders() {
				...
				QueryBackOrders(condition).then(res => {
					...
					AddOperationLog('EXPORT_MUANALORDER', JSON.stringify(condition))
				})
			},
