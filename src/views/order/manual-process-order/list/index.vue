        ...
        <el-button type="text" @click="exportOrders"><br></el-button>
          ...
          <el-link slot="reference" type="info" @click="goHotel(scope.row.hotelCode)">{{ scope.row.hotelName}}</el-link>
            ...
            >
				...
				<el-table-column prop="orderDate" label="下单日期" min-width="6%" v-if="colData[8].istrue" />
				<!-- <el-table-column prop="customerName" label="客户名称" />
        <el-table-column prop="customerNo" label="客户编号" /> -->
				<el-table-column label="订单状态" min-width="4%" v-if="colData[9].istrue">
					<template slot-scope="scope">
						<!-- <span v-if="scope.row.orderStatus == OrderStatusEnum.RESERVEING" color="red">
            {{ scope.row.orderStatusDesc }}
            </span> -->
						<font v-if="scope.row.lockUserName && scope.row.lockUserName.length>0" style="color:#EC9316">
							{{ scope.row.orderStatusDesc }}
						</font>
						<font v-else style="color:red">{{ scope.row.orderStatusDesc }}</font>
          ...
          <template slot-scope="scope">
            <el-tag effect="plain">{{ scope.row.refundStatusDesc }}</el-tag>
				...
				<el-table-column label="操作" min-width="4%" v-if="colData[11].istrue">
					<template slot-scope="scope">
						<el-link style="color:#DF9524" size="mini" @click="goDetails(scope.row.orderNo,1)">详情</el-link>
					</template>
				</el-table-column>
				<el-table-column label="处理时间" min-width="6%" v-if="colData[12].istrue">
					<template slot-scope="scope">
						{{ scope.row.remaining }}
					</template>
				</el-table-column>
			   ...
			   <el-table-column prop="platform" label="数据渠道/开票方" min-width="8%" v-if="colData[15].istrue">
			   	<template slot-scope="scope">
						{{ scope.row.platform }}
					    <span><b v-if="scope.row.invoiceTypeDesc">【{{scope.row.invoiceTypeDesc }}】</b> </span>
					</template>
			   </el-table-column>
				<el-table-column prop="lockUserName" label="处理人" min-width="5%" v-if="colData[14].istrue" />
			</el-table>
...
<script>
	...
	export default {
		name: 'ManualWaitHandleOrderList',
		directives: {
			waves
		},
		components: {
		...
		methods: {
			...
			exportOrders() {
				...
				QueryBackOrders(condition).then(res => {
					...
					AddOperationLog('EXPORT_WAITMUANALORDER', JSON.stringify(condition))
				})
			},
			...
			validForm() {
				...
				if (o.thirdOrderNo == "" && o.orderNo == "" && o.checkInName == "" && o.orderStatus == "" &&
					!(this.date && this.date.length > 0) && o.hotelName == "" && o.checkInPhone == "" && o.platformId ==
					"" && o.customerName == "") {
					this.searchCondition.orderTimes = [this.dateTimeToDate(new Date(new Date() - 24 * 60 * 60 * 1000)),
						this.dateTimeToDate(new Date())
					]
					this.searchCondition1.orderTimes = [this.dateTimeToDate(new Date(new Date() - 24 * 60 * 60 * 1000)),
						this.dateTimeToDate(new Date())
					]
				} else {
					if (!(this.date && this.date.length > 0)) {
						this.searchCondition1.orderTimes = []
						this.searchCondition.orderTimes = []
						this.searchCondition1.orderDates = []
						this.searchCondition.orderDates = []
					}
				}
			},
